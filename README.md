...
- **Timelapses are found on whichever medium recorded them, and the right one is attached.** The printer reports the full path of the recording it has just finished, so that file is taken by name rather than "the first one that was not there before". On internal storage it also says which print each recording belongs to, matching it to the right archive outright.
