#!/usr/bin/env bash
#
# BamDude Docker Installation Script
# Supports: Linux (all distros), macOS
#
# Usage:
#   Interactive:  curl -fsSL https://raw.githubusercontent.com/kainpl/bamdude/main/install/docker-install.sh -o docker-install.sh && chmod +x docker-install.sh && ./docker-install.sh
#   Unattended:   ./docker-install.sh --path /opt/bamdude --port 8000 --yes
#
# Options:
...
show_help() {
    echo "BamDude Docker Installation Script"
    echo ""
    ...
    echo "    ./docker-install.sh --path /srv/bamdude --port 3000 --tz America/New_York --yes"
    echo ""
...
create_install_dir() {
    ...
    if ! mkdir -p "$INSTALL_PATH" 2>/dev/null; then
        # The default `/opt/bamdude` (and any other root-owned parent) needs
...
start_container() {
    ...
    while [[ $attempt -lt $max_attempts ]]; do
        ...
        if $DOCKER_CMD ps | grep -q "Up"; then
            log_success "BamDude container is running"
            return 0
        fi
        ...
        if $DOCKER_CMD ps -a | grep -q "Exited"; then
            ...
            log_info "Check logs with: $DOCKER_CMD logs bamdude"
            return 1
        fi
...
main() {
    ...
    echo -e "    Logs:    cd $INSTALL_PATH && $DOCKER_CMD logs -f bamdude"
