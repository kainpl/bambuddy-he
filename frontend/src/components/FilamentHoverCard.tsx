...
export function FilamentHoverCard({ data, children, disabled, className = '', spoolman, inventory, configureSlot }: FilamentHoverCardProps) {
  ...
  //
  // The card sits at z-[60] so it can escape sibling printer cards' stacking
  // contexts, which puts it ABOVE ConfigureAmsSlotModal and LinkSpoolModal at
  // z-50 — so a card left standing draws over the very dialog it just opened.
  // Mouseleave is the only thing that normally hides it, and a touch device never
  // sends one after the tap that opened the card, so on a tablet it stays up
  // indefinitely (upstream #2631). Clearing the timeout is not optional: a
  // pending show timer would re-open it.
  const dismiss = () => {
    if (timeoutRef.current) clearTimeout(timeoutRef.current);
    setIsVisible(false);
  };
                  ...
                  {spoolman.linkedSpoolId && (
                    <>
                      <button
                        ...
                        className="w-full flex items-center justify-center gap-1.5 px-2 py-1.5 text-xs font-medium rounded transition-colors bg-bambu-green/20 hover:bg-bambu-green/40 text-bambu-green"
                    ...
                    <>
                      ...
                      {inventory.onUnassignSpool && (
                        <button
                          ...
                          className="w-full flex items-center justify-center gap-1.5 px-2 py-1.5 text-xs font-medium rounded transition-colors bg-red-100 dark:bg-red-500/20 hover:bg-red-200 dark:hover:bg-red-500/40 text-red-700 dark:text-red-400"
                        >
...
export function EmptySlotHoverCard({ children, className = '', configureSlot, onAssignSpool, emptyKind }: EmptySlotHoverCardProps) {
  ...
  // See FilamentHoverCard.dismiss — same z-[60]-over-a-z-50-dialog problem, and
  // the same missing mouseleave on touch (upstream #2631).
  const dismiss = () => {
    if (timeoutRef.current) clearTimeout(timeoutRef.current);
    setIsVisible(false);
  };
        ...
        >
          ...
          ">
            <div className="px-3 py-1.5 text-xs whitespace-nowrap">
              {emptyKind === 'reset' ? (
                <span className="text-amber-700 dark:text-amber-400">{t('ams.slotLoadedUnconfigured')}</span>
              ) : (
            ...
            {(configureSlot?.enabled || onAssignSpool) && (
              <div className="px-2 pb-2 space-y-1">
                {/* ⚠️ Assign BEFORE Configure, matching the filled-slot card
                    above. The two cards are separate render paths and had
                    drifted into opposite orders, so the menu reshuffled itself
                    depending on whether the slot happened to hold filament. */}
                {onAssignSpool && (
                  <button
