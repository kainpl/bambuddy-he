...
export function EmbeddedCameraViewer({ printerId, printerName, viewerIndex = 0, onClose }: EmbeddedCameraViewerProps) {
  ...
  // Camera diagnostic modal — opens from both the always-visible
  // stethoscope icon in the control bar AND the error-state Diagnose
  // button. Cuts off the "user opens a 'camera broken' ticket → wait
  // → triage" loop at the user's screen (upstream Bambuddy #1395
  // follow-up).
