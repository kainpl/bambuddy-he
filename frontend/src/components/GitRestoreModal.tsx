 ...
 * GitBackupSettings — including the `defaultValue` arm, which is what keeps a
...
function translateCoded(
  ...
  params: GitRestoreParams | undefined,
...
export function GitRestoreModal({ onClose }: GitRestoreModalProps) {
  ...
  const [result, setResult] = useState<GitRestoreResponse | null>(null);

  const commitsQuery = useQuery({
    queryKey: ['github-backup-commits'],
    queryFn: () => api.getGitBackupCommits(20),
  });

  const previewQuery = useQuery({
    queryKey: ['github-restore-preview', selectedRef],
    queryFn: () => api.getGitRestorePreview(selectedRef),
  });
        ...
        header={
          <>
            ...
            <div className="min-w-0">
              <h3 id={headingId} className="text-lg font-semibold text-white">{t('backup.restoreFromGit.title')}</h3>
