import { useEffect, useLayoutEffect, useRef, useState, useCallback } from 'react';
import { useTranslation } from 'react-i18next';
import {
  Search, X, ArrowUpNarrowWide, ArrowDownWideNarrow,
  Wrench, History, Settings, Filter, SlidersHorizontal,
} from 'lucide-react';

type TabType = 'status' | 'history' | 'settings';
type StatusFilter = 'all' | 'due' | 'warning' | 'ok';
type SortOption = 'upcoming' | 'name' | 'hours' | 'location';

interface MaintenanceToolbarProps {
  activeTab: TabType;
  onTabChange: (tab: TabType) => void;

  // Filters / sort (only meaningful when activeTab === 'status'; the toolbar
  // hides those controls when not).
  search: string;
  onSearchChange: (value: string) => void;

  statusFilter: StatusFilter;
  onStatusFilterChange: (value: StatusFilter) => void;

  locationFilter: string;
  onLocationFilterChange: (value: string) => void;
  /** The tree, flattened: `depth` drives the indent and `path` the title.
   *  Ids, not names — a name stopped being an identity when "Shelf 1"
   *  became possible under two workshops. */
  availableLocations: { id: number; label: string; depth: number; path: string }[];

  sortBy: SortOption;
  onSortByChange: (value: SortOption) => void;
  sortAsc: boolean;
  onSortDirectionToggle: () => void;

  hideOffline: boolean;
  onHideOfflineToggle: () => void;
}

const TAB_BUTTONS: { tab: TabType; labelKey: string; icon: typeof Wrench }[] = [
  { tab: 'status', labelKey: 'maintenance.tabs.status', icon: Wrench },
  { tab: 'history', labelKey: 'maintenance.tabs.history', icon: History },
  { tab: 'settings', labelKey: 'maintenance.tabs.settings', icon: Settings },
];

/** Local overflow popover (same shell as QueueToolbar's OverflowMenu). */
function OverflowMenu({
  label,
  icon,
  children,
}: {
  label: string;
  icon: React.ReactNode;
  children: React.ReactNode;
}) {
  const [isOpen, setIsOpen] = useState(false);
  return (
    <div className="relative">
      <button
        type="button"
        onClick={() => setIsOpen((v) => !v)}
        className="h-8 w-8 rounded-lg border bg-bambu-dark border-bambu-dark-tertiary text-white hover:bg-bambu-dark-tertiary transition-colors flex items-center justify-center"
        aria-label={label}
        title={label}
      >
        {icon}
      </button>
      {isOpen && (
        <>
          {/* not-a-modal: menu */}
          <div className="fixed inset-0 z-10" onClick={() => setIsOpen(false)} />
          <div
            className="absolute right-0 top-full z-20 mt-1 min-w-48 rounded-lg border border-bambu-dark-tertiary bg-bambu-dark-secondary p-2 shadow-xl"
            onClick={() => setIsOpen(false)}
          >
            {children}
          </div>
        </>
      )}
    </div>
  );
}

/**
 * In-header toolbar for MaintenancePage — matches the QueuePage / PrintersPage
 * shape. Hosts search + status/location filters + hide-offline + sort + a tab
 * segmented control (Status / History / Settings).
 *
 * Filter / sort / hide-offline controls only apply to the Status tab and are
 * hidden when the active tab is History or Settings. The tab segmented control
 * stays visible regardless.
 *
 * On narrow viewports the inline ribbon collapses to two icon-only overflow
 * menus (Filters / View) via a ResizeObserver against an off-screen mirror.
 */
export function MaintenanceToolbar({
  activeTab,
  onTabChange,
  search,
  onSearchChange,
  statusFilter,
  onStatusFilterChange,
  locationFilter,
  onLocationFilterChange,
  availableLocations,
  sortBy,
  onSortByChange,
  sortAsc,
  onSortDirectionToggle,
  hideOffline,
  onHideOfflineToggle,
}: MaintenanceToolbarProps) {
  const { t } = useTranslation();
  const isStatusTab = activeTab === 'status';

  const wrapperRef = useRef<HTMLDivElement | null>(null);
  const expandedRef = useRef<HTMLDivElement | null>(null);
  const [compact, setCompact] = useState(false);

  const measure = useCallback(() => {
    const wrap = wrapperRef.current;
    const expanded = expandedRef.current;
    if (!wrap || !expanded) return;
    const needed = expanded.scrollWidth;
    const available = wrap.clientWidth - 240;
    setCompact(needed > available);
  }, []);

  useLayoutEffect(() => {
    measure();
  }, [measure, availableLocations.length, isStatusTab]);

  useEffect(() => {
    const wrap = wrapperRef.current;
    if (!wrap) return;
    const ro = new ResizeObserver(() => measure());
    ro.observe(wrap);
    window.addEventListener('resize', measure);
    return () => {
      ro.disconnect();
      window.removeEventListener('resize', measure);
    };
  }, [measure]);

  const renderFilterControls = (inMenu = false) => {
    const fullWidth = inMenu ? 'w-full' : '';
    return (
      <>
        <select
          value={statusFilter}
          onChange={(e) => onStatusFilterChange(e.target.value as StatusFilter)}
          className={`h-8 text-sm bg-bambu-dark border border-bambu-dark-tertiary rounded-lg px-2 text-white focus:border-bambu-green focus:outline-none ${fullWidth}`}
        >
          <option value="all">{t('maintenance.filter.all')}</option>
          <option value="due">{t('maintenance.filter.due')}</option>
          <option value="warning">{t('maintenance.filter.warning')}</option>
          <option value="ok">{t('maintenance.filter.ok')}</option>
        </select>

        {availableLocations.length > 0 && (
          <select
            value={locationFilter}
            onChange={(e) => onLocationFilterChange(e.target.value)}
            className={`h-8 text-sm bg-bambu-dark border border-bambu-dark-tertiary rounded-lg px-2 text-white focus:border-bambu-green focus:outline-none ${fullWidth}`}
          >
            <option value="all">{t('printers.filter.allLocations')}</option>
            {availableLocations.map((loc) => (
              <option key={loc.id} value={String(loc.id)} title={loc.path}>
                {' '.repeat((loc.depth - 1) * 3)}
                {loc.label}
              </option>
            ))}
          </select>
        )}

        <button
          type="button"
          onClick={onHideOfflineToggle}
          aria-pressed={hideOffline}
          className={`h-8 px-2 rounded-lg border text-sm font-medium transition-colors ${fullWidth} ${
            hideOffline
              ? 'bg-bambu-green border-bambu-green text-white'
              : 'bg-bambu-dark border-bambu-dark-tertiary text-white hover:bg-bambu-dark-tertiary'
          }`}
        >
          {t('printers.hideOffline')}
        </button>
      </>
    );
  };

  const renderSortControls = (inMenu = false) => {
    const fullWidth = inMenu ? 'w-full' : '';
    return (
      <div className={`flex items-center gap-1 ${fullWidth}`}>
        <select
          value={sortBy}
          onChange={(e) => onSortByChange(e.target.value as SortOption)}
          className={`h-8 text-sm bg-bambu-dark border border-bambu-dark-tertiary rounded-lg px-2 text-white focus:border-bambu-green focus:outline-none ${inMenu ? 'flex-1' : ''}`}
        >
          <option value="upcoming">{t('maintenance.sort.upcoming')}</option>
          <option value="name">{t('maintenance.sort.name')}</option>
          <option value="hours">{t('maintenance.sort.hours')}</option>
          <option value="location">{t('maintenance.sort.location')}</option>
        </select>
        <button
          type="button"
          onClick={onSortDirectionToggle}
          className="h-8 w-8 shrink-0 flex items-center justify-center bg-bambu-dark border border-bambu-dark-tertiary rounded-lg hover:border-bambu-green transition-colors"
          title={sortAsc ? t('printers.sort.descending') : t('printers.sort.ascending')}
        >
          {sortAsc
            ? <ArrowUpNarrowWide className="w-4 h-4 text-white" />
            : <ArrowDownWideNarrow className="w-4 h-4 text-white" />}
        </button>
      </div>
    );
  };

  /**
   * Tab segmented control. Always rendered inline (tabs are primary
   * navigation — never collapsed into an overflow menu). In compact mode
   * the labels drop and we keep icon-only buttons so the row still fits.
   */
  const renderTabControl = (iconOnly = false) => (
    <div className="flex h-8 items-center bg-bambu-dark rounded-lg border border-bambu-dark-tertiary">
      {TAB_BUTTONS.map(({ tab, labelKey, icon: Icon }, index) => {
        const isSelected = activeTab === tab;
        const label = t(labelKey);
        return (
          <button
            key={tab}
            type="button"
            onClick={() => onTabChange(tab)}
            className={`h-full ${iconOnly ? 'w-9' : 'px-3'} text-xs font-medium transition-colors flex items-center justify-center gap-1.5 ${
              index === 0 ? 'rounded-l-lg' : ''
            } ${
              index === TAB_BUTTONS.length - 1 ? 'rounded-r-lg' : ''
            } ${
              isSelected
                ? 'bg-bambu-green text-white'
                : 'text-bambu-gray hover:bg-bambu-dark-tertiary hover:text-white'
            }`}
            title={label}
            aria-label={label}
          >
            <Icon className="w-3.5 h-3.5" />
            {!iconOnly && label}
          </button>
        );
      })}
    </div>
  );

  return (
    <div ref={wrapperRef} className="relative flex items-center gap-2">
      {/* Search — only useful on Status tab. Always rendered so the layout
          doesn't jump when switching tabs; disabled when not on Status. */}
      <div className="relative min-w-0 flex-1">
        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-bambu-gray/50" />
        <input
          type="search"
          autoComplete="off"
          value={search}
          onChange={(e) => onSearchChange(e.target.value)}
          placeholder={t('maintenance.search.placeholder')}
          aria-label={t('maintenance.search.placeholder')}
          disabled={!isStatusTab}
          className="w-full h-8 pl-9 pr-8 bg-bambu-dark border border-bambu-dark-tertiary rounded-lg text-white text-sm placeholder:text-bambu-gray/50 focus:outline-none focus:border-bambu-green disabled:opacity-40 disabled:cursor-not-allowed"
        />
        {search && isStatusTab && (
          <button
            type="button"
            aria-label={t('common.clear')}
            onClick={() => onSearchChange('')}
            className="absolute right-2 top-1/2 -translate-y-1/2 text-bambu-gray hover:text-white"
          >
            <X className="w-4 h-4" />
          </button>
        )}
      </div>

      {/* Expanded inline controls — visible when wide enough; otherwise kept
          mounted off-screen so its scrollWidth stays measurable. */}
      <div
        ref={expandedRef}
        aria-hidden={compact}
        // @ts-expect-error -- `inert` is a valid HTML attribute, TS lib.dom hasn't picked it up universally
        inert={compact ? '' : undefined}
        className={`${compact ? 'absolute -left-[9999px] top-0 flex w-max pointer-events-none opacity-0' : 'flex'} ml-auto items-center gap-2 flex-nowrap [&>*]:shrink-0`}
      >
        {isStatusTab && (
          <>
            <div className="flex items-center gap-2">{renderFilterControls()}</div>
            <div className="h-6 w-px bg-bambu-dark-tertiary" />
            {renderSortControls()}
            <div className="h-6 w-px bg-bambu-dark-tertiary" />
          </>
        )}
        {renderTabControl()}
      </div>

      {/* Compact overflow menus — Filters / View grouped icons when ribbon
          overflows. Filters/View are hidden when not on Status tab (nothing
          for them to do). Tab control is always rendered inline. */}
      {compact && (
        <div className="ml-auto flex items-center gap-1">
          {isStatusTab && (
            <>
              <OverflowMenu label={t('queue.toolbar.filters')} icon={<Filter className="w-4 h-4" />}>
                <div className="flex w-48 flex-col gap-2">{renderFilterControls(true)}</div>
              </OverflowMenu>
              <OverflowMenu label={t('queue.toolbar.view')} icon={<SlidersHorizontal className="w-4 h-4" />}>
                <div className="flex w-48 flex-col gap-2">{renderSortControls(true)}</div>
              </OverflowMenu>
            </>
          )}
          {renderTabControl(true)}
        </div>
      )}
    </div>
  );
}
