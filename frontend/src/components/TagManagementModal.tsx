...
export function TagManagementModal({ onClose }: TagManagementModalProps) {
  ...
  // Escape unwinds one layer at a time — the rename editor or the delete
  // confirmation first, the dialog only when nothing is open inside it.
  // Closing the dialog is the shell's job (the stack listens on `window`), so
  // this one takes the event only while something is open INSIDE, on
  // `document`, and stops it there.
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key !== 'Escape') return;
      if (!editingTag && !deleteConfirm) return;
      e.stopPropagation();
  ...
  return (
    <Modal
      onClose={onClose}
      title="Manage Tags"
      icon={<Tag className="w-5 h-5 text-bambu-green" />}
      size="lg"
