...
export interface PrinterCreate {
  ...
  // disconnects or reconnects MQTT accordingly.
  is_active?: boolean;
  cleanup_after_print?: boolean;
  mqtt_connection_timeout?: number;
  external_camera_url?: string | null;
...
export interface AppSettings {
  ...
  // BAMDUDE_LOCAL_LOGIN=true on the server flips the reported value back to
  ...
  // Session policy (#1706, adapted): admin ceiling on session lifetime in hours (default 720 = 30d)
...
export interface SmartPlug {
  ...
  // #1349: auto-off after AMS drying. Independent of auto_off (print-finish).
...
export interface AdvancedAuthStatus {
  ...
  local_login_enabled: boolean;
  // #1589: when set, LoginPage redirects to this provider's authorize URL on
  // mount unless ?fallback=local is in the URL or the redirect times out.
...
export const api = {
  ...
  // until the status flips to 'complete' (or a terminal deny/expire). The
  // device_code itself never reaches the browser — it stays server-side.
  ...
  exportSpoolsCsv: async (options?: CsvExportOptions): Promise<void> => {
    ...
    const disposition = response.headers.get('Content-Disposition');
    const filename = parseContentDispositionFilename(disposition) || 'bamdude_inventory.csv';
    const blob = await response.blob();
    const url = window.URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
  ...
  applyUpdate: (tagName?: string) =>
    request<{ success: boolean; message: string; target_ref?: string; status?: UpdateStatus; is_docker?: boolean; is_ha_addon?: boolean; is_windows_installer?: boolean }>('/updates/apply', {
      method: 'POST',
  ...
  // summary_code in i18n.
  diagnoseCamera: (printerId: number) =>
    request<CameraDiagnoseResult>(`/printers/${printerId}/camera/diagnose`, { method: 'POST' }),
  // Connection diagnostic (#1465 / triage): GET runs it for a saved printer,
  // POST runs the pre-save Add-Printer variant. Each check's id + status
  // resolve a localized title/fix under `diagnostic.check.*`.
  ...
  checkPlateEmpty: (printerId: number, options?: { useExternal?: boolean; includeDebugImage?: boolean }) => {
    const params = new URLSearchParams();
    // #1359: only forward use_external when the caller explicitly sets
    // it. Omitted → backend derives the default from the printer's
    // external_camera_enabled setting so calibration and runtime checks
    // use the same camera.
  ...
  calibratePlateDetection: (printerId: number, options?: { label?: string; useExternal?: boolean }) => {
    ...
    // #1359: only forward use_external when explicitly set; backend
    // defaults from printer.external_camera_enabled otherwise.
    if (options?.useExternal !== undefined) {
      params.set('use_external', String(options.useExternal));
    }
  ...
  // Printer heater (nozzle / bed / chamber) history
