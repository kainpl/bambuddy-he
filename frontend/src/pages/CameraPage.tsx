...
export function CameraPage() {
  ...
  // Consecutive "stalled/inactive" status reads. We only reconnect after two in
  // a row so a brief blip while the shared fan-out upstream is starting up or
  // handing over between viewers doesn't tear down a stream that's about to
  // deliver frames — that churn is what stranded the P1S camera black for
  ...
  useEffect(() => {
    ...
    stallCheckIntervalRef.current = setInterval(async () => {
      try {
        ...
        // Require two consecutive bad reads before acting (upstream #2521) —
        // one blip during fan-out startup/handover is not a real stall.
  ...
  const handleStreamLoad = () => {
    setStreamLoading(false);
    setStreamError(false);
    // A frame rendered — clear any accumulated stall strikes (upstream #2521).
    stallStrikesRef.current = 0;
