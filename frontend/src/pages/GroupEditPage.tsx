...
export function GroupEditPage() {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: (updatedGroup) => {
      ...
      // (#1083). `setQueryData` alone is enough — invalidating ['group', id]
      // here would trigger an immediate background refetch that could race
      // with / overwrite this primed value in test environments where the
      // GET handler is a static mock; in production the server's GET would
      // match this payload anyway.
      if (updatedGroup) {
