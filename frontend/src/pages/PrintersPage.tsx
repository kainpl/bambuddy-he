...
function PrinterCard({
  ...
  // Runout / filament-replacement guidance (upstream #2587). The backend fills
  // these only while the print is PAUSED (global tray IDs, or null when idle /
  // unresolvable):
                          ...
                          <div key={ams.id} className="p-2.5 bg-bambu-dark rounded-lg border border-bambu-dark-tertiary/30">
                            ...
                            <div className="grid grid-cols-4 gap-1.5">
                              {[0, 1, 2, 3].map((slotIdx) => {
                                ...
                                // Runout guidance (upstream #2587): the slot the paused
                                ...
                                const slotVisual = (
                                  <div
                                    className={`relative bg-bambu-dark-tertiary rounded p-1 text-center ${isEmpty ? 'opacity-50' : ''} ${
                                      isExpectedSlot
...
export function PrintersPage() {
  ...
  // Fetch curated UI-rendering settings (AMS thresholds, camera view mode,
  // time/date format, drying presets, …). Uses the no-SETTINGS_READ
  // /ui-preferences endpoint so non-admin operators don't 403 and silently
  // lose these UI features (#1293). Separate cache key from the admin-gated
  // full settings query.
