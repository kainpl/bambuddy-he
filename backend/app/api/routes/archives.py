...
def _match_timelapse_by_timestamp(
    ...
    *,
    ...
    ``video_2026-05-08_09-41-29.mp4``). The printer clock may be offset from the
    server's — especially in LAN-Only mode where NTP is unreachable — so we try a
    small set of common UTC offsets and keep the (video, offset) pair closest to
    ``archive_start``. We deliberately do NOT compare the filename against the
    archive's end time: the filename is start time, not end time, so end-time
    proximity is not a real signal (Strategy 3 handles end via file mtime).

    Because the offset list densely covers a wide span of the day, an unrelated
    video's filename can coincidentally land near a later print's start at some
    offset. To reject that false positive, the best (video, offset) pair must beat
    the next-best pair *from a different video* by at least ``ambiguity_margin``;
    when the top two candidates from different videos are too close to call we
    return ``(None, None)`` and let the caller fall back to manual selection
    (upstream Bambuddy #1278 / commit 4ee4bdb0).
    """
    import re
    ...
    if ambiguity_margin is None:
        ambiguity_margin = timedelta(minutes=15)
    ...
    for f in video_files:
        ...
        for hour_offset in offsets_hours:
            diff = abs((file_time - timedelta(hours=hour_offset)) - archive_start)
            if diff <= tolerance:
...
async def scan_timelapse(
    ...
    # local clock. See _match_timelapse_by_timestamp for the offset-search
    # rationale and why we intentionally don't match the filename against end
    # time here (that produced the #1278 false positive).
...
def _resolve_source_3mf_path(archive: PrintArchive, source_filename: str) -> Path:
    ...
    but BamDude never saw the source 3MF — cloud / Handy / pre-existing
    ...
    raised a 500 on the final ``relative_to`` (#1531). Fallback archives now
    land under ``<base_dir>/archive/no_source/<archive_id>/`` instead, which
    stays inside the data volume and remains addressable by every read site
    that does ``base_dir / archive.source_3mf_path``.
    ...
    or manual SQL edit fails with a clear 500 instead of writing outside the
    data volume.
    """
    if archive.file_path:
    ...
    # return the *literal* source_dir below. Resolving the returned path would
    # canonicalise away a symlinked DATA_DIR (legitimate on TrueNAS / QNAP /
    # Synology storage pools, and any ``-v /symlink:/app/data`` mount), which
    # would then make the caller's ``source_path.relative_to(settings.base_dir)``
    # raise because the left side is canonical and the right is the symlink path.
    try:
...
async def slice_archive(
    ...
    # caller slice another user's archive by raw id even though GET on that id
