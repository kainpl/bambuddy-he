...
async def _api_key_to_user_response(db: AsyncSession, api_key) -> UserResponse:
    """What ``/auth/me`` tells a caller holding an API key.

    ⚠️ This used to answer with a synthetic administrator: ``id=0``,
    ``role="admin"``, ``is_admin=True`` and every permission in the enum. That
    was wrong in both directions. A key cannot perform administrative
    operations at all — the gate denies every permission outside the scope
    allowlist — so a client building its UI from this response (which is
    exactly what a native client does) rendered admin actions that 403 on use,
    and had no way to learn the id its own prints are filed under.

    Now: identity comes from the key's owner, and ``permissions`` is the set
    the key can genuinely exercise. ``is_admin`` is always False because no key
    reaches an administrative route regardless of who owns it.
    ...
    """
    # ⚠️ The same resolution the permission gate uses, so what is reported here
    # and what is enforced there cannot drift — including the 403 when the
    # owner has been deactivated, which makes the key dead rather than
    # anonymous.
    ...
    return UserResponse(
        ...
        email=None,
        # Deprecated free-text field; "user" is the existing value meaning "not
        # an admin". Inventing an "api_key" role here would put a third value
        # into a field callers compare against string literals.
        role="user",
        is_active=True,
        is_admin=False,
...
async def search_ldap_directory(
    ...
    Returns up to 25 candidates. The query is matched (case-insensitively,
    with wildcards on both sides) against sAMAccountName, uid, mail,
    displayName, and cn — covering both AD and OpenLDAP layouts. Each
    result is annotated with ``already_provisioned`` so the UI can grey
    out usernames that already exist as BamDude users.

    Requires USERS_CREATE. Minimum query length is 2 characters.
    """
    import logging
    ...
    try:
        ...
        logger.exception("LDAP directory search failed")
        # Admin-only endpoint — surface the underlying reason so the
        # operator can fix it (USERS_CREATE permission already gated).
        raise HTTPException(
    ...
    # Annotate ``already_provisioned`` so the SPA can dim / disable rows
    # that map to an existing local row. Case-insensitive lookup mirrors
    # ``create_user``.
...
async def provision_ldap_user(
    ...
    """Provision a BamDude user from an existing LDAP directory entry.

    Re-resolves the username via the service-account bind (rather than
    trusting the request body) so group mappings and email come from a
    fresh LDAP read. Applies the same group-mapping / default-group logic
    as the auto-provision login path (``_provision_ldap_user``), so
    behaviour stays identical regardless of whether the user was created
    here or on first login. Requires USERS_CREATE.
    """
    import logging
    ...
    # Look up via service bind. Service-bind failures bubble up as 503;
    # missing entries surface as 404 to distinguish "directory
    # unreachable" from "username doesn't exist in the directory" in the
    # UI.
    try:
        ldap_user = lookup_ldap_user(config, username)
    except Exception as e:
        logger.exception("LDAP lookup failed during provision")
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
    ...
    # Reject duplicates — the canonical username from LDAP is what gets
    ...
    # Reload with groups eagerly loaded so ``_user_to_response`` can
    # serialise them without lazy-load warnings (matches ``create_user`` /
    # ``list_users`` pattern).
    result = await db.execute(select(User).where(User.id == new_user.id).options(selectinload(User.groups)))
    new_user = result.scalar_one()
    logger.info("Manually provisioned LDAP user %s (id=%d)", new_user.username, new_user.id)
...
async def get_encryption_status(
    ...
    Gated on SETTINGS_UPDATE so Viewers (SETTINGS_READ only) cannot read the
    encryption status — admin/operator only.
    """
    import logging as _logging
    ...
    try:
        ...
        _enc_logger.exception("Failed to query encryption row counts")
    ...
    # Detect "wrong key" state — sample-decrypt one encrypted row to
    # distinguish "no key" from "key configured but cannot decrypt these
    # rows" (rotation / cross-deployment restore / wrong env override).
    ...
    if not key_configured and total_encrypted > 0:
        ...
        try:
            ...
            _enc_logger.exception("Failed to query sample encrypted row for decryption probe")
