    ...
    for printer in printers_list:
        ...
        for ams_unit in status.raw_data.get("ams", []):
            ...
            for tray in ams_unit.get("tray", []):
                ...
                # (#1545). Opaque colours still round-trip as #RRGGBB. The dedup
                # key uses the 6-char RGB so two slots that share an RGB but
                # differ only in alpha still merge.
...
async def get_printer_status(
    ...
    # cache to render "<filament> @ <temp>°C".
...
async def get_overlay_status(
    printer_id: int,
    _: None = RequireOverlayToken,
    db: AsyncSession = Depends(get_db),
) -> dict:
    """Everything the streaming overlay draws for one printer (upstream #2613).
    ...
    the one setting the overlay reads) rather than the full ``PrinterStatus``: a
    token holder gets exactly the fields the overlay renders, nothing more — no
    serial number, no IP address.

    It *does* include the print filename, because the overlay names the part on
    screen. That is why it sits behind its own ``overlay`` scope instead of the
    ``camera_stream`` scope already handed out for video alone.
    """
...
async def _load_printer_or_404(printer_id: int) -> Printer:
    ...
    The file-manager and storage routes talk FTP to the printer, which can block
    for the full socket timeout — longer when a saturated FTP pool backs up.
    Holding the request's Depends(get_db) session across that FTP pinned one
    ...
    the printer doesn't exist. Reference async_session via the module so the
    maker is resolved at call time (stays in sync with reinitialize_database()
    + test patches).
    """
...
async def configure_ams_slot(
    ...
    if not await printer_manager.ensure_fresh_connection(printer_id):
        raise HTTPException(status_code=500, detail="Can`t re-connect printer MQTT")
...
async def stop_print(
    ...
    # re-Connect MQTT if stalled
    if not await printer_manager.ensure_fresh_connection_for_printer(printer):
        raise HTTPException(400, "Can`t re-connect printer MQTT")
...
async def execute_hms_action(
    ...
    #
    # We probe `_last_message_time` (bumped on every inbound MQTT push regardless of
    # payload) rather than a (gcode_state, hms_errors-length) diff. The old diff missed
    # the wrong-plate IGNORE_RESUME case (#1869) where the printer briefly resumes and
    # re-pauses with the same fault inside the 2.5s window: both fields round-trip to
    # their pre-publish values → false 502 even though the firmware fully ack'd. Every
    # accepted command triggers a pushall response within ~100-500ms, so a fresh inbound
    # message after the publish is the robust ack signal.
    ...
    if not acked:
        # Publish succeeded but the printer sent nothing back. Almost always firmware-side
        # silent rejection (err mismatch, command/state mismatch) or a dropped MQTT route.
        # 502 makes it visible at the UI instead of the 200-but-broken loop #1830 reported.
