...
async def capture_frame(
    ...
    Args:
        ...
        snapshot_url: Optional override for single-frame capture. When set, fetched
            ...
            ``/api/frame.jpeg`` reliably returns a clean image while the MJPEG
            stream's first frame is often the encoder's stale keyframe). Empty
            string is treated as unset (defence in depth — a stale config row that
            somehow has ``""`` rather than ``NULL`` still routes through the live
            stream rather than firing GET against an empty URL). Upstream Bambuddy
            #1177.
...
async def _capture_mjpeg_frame(url: str, timeout: int) -> bytes | None:
    ...
    snapshot UX, plate-detect CV, finish photo, layer timelapse, Obico ML
    inference).
