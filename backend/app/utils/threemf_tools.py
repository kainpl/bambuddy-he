...
# stdlib one, and defusedxml does not re-export it, so annotations name it
# directly.
...
# Bound on the dense slot list. The slot IDs are whatever the file says, so a
# corrupt or hostile 3MF declaring `filament id="50000000"` would otherwise
# allocate a fifty-million-entry list on the dispatch path.
_MAX_DENSE_FILAMENT_SLOTS = 64


def extract_slot_extruders_from_3mf(file_path: Path, plate_id: int | None = None) -> list[int] | None:
    """Per-slot extruder assignment as a dense list, or None.

    Same data as :func:`extract_nozzle_mapping_from_3mf`, reshaped for the
    dispatcher: index 0 is filament slot 1, and a slot this file does not print
    is ``-1``. Nozzle-rack printers (H2C) need it to build the physical
    ``nozzle_mapping`` the firmware expects — without one they fall back to
    picking a nozzle themselves, which can level with one hotend and print with
    another, several millimetres off the bed (upstream #2800).

    ``plate_id`` scopes the answer to the plate actually being dispatched. A
    multi-plate 3MF carries one filament list per plate and they need not agree,
    so without it a slot can take its extruder from a plate this print is not
    going to run.

    Takes a path rather than an open archive because the dispatcher is handling
    the file, not the zip, and a broken file there must not take the print down:
    an unreadable or non-3MF path returns None, and the caller dispatches
...
def _plates_in_scope(si_root: XmlElement, plate_id: int | None) -> list[XmlElement]:
    ...
    INSIDE ``<plate>``, never a ``plate_idx`` attribute — the same trap that
    made ``plate_number`` inert for months.
    """
...
def _group_extruder_indices(plates: list[XmlElement]) -> dict[int, int] | None:
    ...
    ``slice_info.config`` states this directly, as
    ``<nozzle id="<group>" extruder_id="<1-based extruder>"/>``.

    ⚠️ **Reading it is what makes a nozzle-rack printer work.** On every other
    dual-nozzle Bambu the group id IS the extruder index, so it was read as one.
    On the H2C it is not: the rack carriage can host six hotends to the fixed
    carriage's one, so the slicer emits a group per *nozzle* rather than per
    carriage, and a plate can carry groups 0, 1 and 2 against a two-entry
    ``physical_extruder_map``.

    Returns None when the file states no table, or when two plates in scope
    disagree about a group — the caller then keeps treating the group id as the
    extruder index, which is what every H2D file wants and what this module has
    always done.
    """
    table: dict[int, int] = {}
    for plate in plates:
...
def extract_nozzle_mapping_from_3mf(zf: zipfile.ZipFile, plate_id: int | None = None) -> dict[int, int] | None:
    ...
    Priority:
        1. group_id on <filament> elements in slice_info.config (actual
           assignment), resolved through the file's own group-to-extruder table
    ...
    ⚠️ **Returns None rather than a partial answer** whenever a filament the
    plate prints cannot be placed. The gap does not stay a gap downstream: the
    dense form fills it with -1, which already means "slot not printed", and
    dispatching that against an ``ams_mapping`` that *does* name a tray for the
    slot is a contradiction the firmware rejects outright with HMS 0500-4047,
    "the available hotend quantity or model does not match the sliced file".
    Giving no mapping at all costs only the firmware's own nozzle pick.
