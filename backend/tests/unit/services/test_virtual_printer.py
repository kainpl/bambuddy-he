...
class TestVirtualPrinterInstance:
    ...
    # ========================================================================
    # #1658 — re-fire FINISH after project_file ack so the slicer releases
    # ========================================================================
    ...
    async def test_on_print_command_proxy_mode_does_not_reschedule_finish(self, instance):
        ...
        PREPARE / RUNNING transition, so the scheduler is exempt."""
        instance.mode = "proxy"
        instance._mqtt = MagicMock()
    ...
    async def test_on_print_command_no_mqtt_does_not_schedule(self, instance):
        """If the MQTT server isn't running yet (transient boot race), the
        scheduler must skip silently — no AttributeError, no orphan task."""
    ...
    async def test_schedule_finish_release_cancels_previous_timer(self, instance):
        """A slicer that fires project_file twice (retry after a transient FTP
        hiccup) must only result in one FINISH — the earlier in-flight timer
        is cancelled when the next one is scheduled."""
        instance.mode = "print_queue"
        instance._mqtt = MagicMock()
    ...
    async def test_delayed_finish_release_sets_finish_state(self, instance):
        ...
        FINISH and prepare_percent=100, the wire-format the slicer's Print flow
        consumes to release "Downloading"."""
        instance._mqtt = MagicMock()
        instance._mqtt.set_gcode_state = MagicMock()
    ...
    async def test_on_print_command_no_filename_does_not_schedule(self, instance):
        """A project_file command without a filename (defensive — real slicers
        always send one) must not schedule a no-op FINISH that would carry an
        empty filename on the next 1 Hz push."""
        instance.mode = "print_queue"
        instance._mqtt = MagicMock()
