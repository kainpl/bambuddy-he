"""Shared test fixtures for BamDude backend tests."""
...
def mfa_encryption_isolation(monkeypatch, tmp_path):
    ...
    - With ``DATA_DIR`` pointing at a writable ``tmp_path``, the default
      ...
      (key_source='generated'), NOT plaintext fallback. Tests that need
      the plaintext fallback path must monkeypatch ``_load_or_generate_key``
      to return ``(None, 'none')`` (or ``'none_write_failed'`` /
      ``'none_corrupted'``) explicitly — see
      ``test_plaintext_passthrough_without_key`` for an example.
